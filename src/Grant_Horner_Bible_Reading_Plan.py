...
from util.const import (
    BIBLE_BOOK_NUMBER_TO_ENGLISH_TINY_ABBREVIATION as eBible_abbreviations,
)
from util.const import (
    BIBLE_BOOK_NUMBER_TO_ENGLISH_SHORT_ABBREVIATION as English_Book_names,
)
...
def get_next_reading_for_list(list_index: int, Book_index: int, chapter_number: int):
...
def get_reading_for_day(
    ...
    for list_index in range(10):
        ...
        while current_day != day_number:
            ...
            Book_index, chapter_number = get_next_reading_for_list(
                list_index, Book_index, chapter_number
            )
...
def get_Bolls_reading_link(
    Book: int = 1,
    chapter: int = 12,
    translation: BollsTranslations = BollsTranslations.Homenko,
):
...
def get_formatted_link(
    ...
    if language == "UK":
        ...
        Book_name = English_Book_names[Book_number]
    elif language == "Language.DE":
        Book_name = German_Book_names[Book_number]

    if link_type == "MDI":
        link = reading_link
    elif link_type == "MDE":
        link = f"[{Book_name} {chapter_number}]({reading_link})"
    elif link_type == "HTML":
        link = f'<a href="{reading_link}">{Book_name} {chapter_number}</a>'
    elif link_type == "NO":
        link = f"{Book_name} {chapter_number}"
